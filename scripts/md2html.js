    ...
    ppt_cute: {
        ...
        render: (slides) => {
            const slidesHtml = slides.map((s, i) => {
                ...
                return `<div class="slide${i===0?' active':''}" data-index="${i}"><div class="slide-content">
...
const styles = {
    ppt: {
        name: 'PPT 幻灯片',
        css: `*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;overflow:hidden}body{font-family:'Noto Sans SC',sans-serif;background:#000;color:#fff}.slides-container{height:100vh;overflow:hidden;position:relative}.slide{height:100vh;display:block;padding:4rem 2rem;overflow-y:auto;align-items:center;padding:4rem;text-align:center;opacity:0;position:absolute;width:100%;transition:opacity .5s ease}.slide.active{opacity:1}.slide h1{font-size:3.5rem;font-weight:700;margin-bottom:2rem}.slide h2{font-size:2.5rem;color:#ff6b6b;margin-bottom:1.5rem}.slide h3{font-size:1.8rem;margin-bottom:1rem}.slide p,.slide ul,.slide ol{font-size:1.3rem;max-width:800px;line-height:1.8;color:#aaa}.slide img{max-height:60vh;object-fit:contain}.slide ul,.slide ol{text-align:center;font-size:1.2rem}.slide li{margin:.8rem 0}.slide code{background:#333;padding:.3em .6em;border-radius:4px}.slide pre{background:#1a1a1a;padding:1.5rem;border-radius:8px;text-align:center;overflow:auto;max-width:90%}.nav-dots{position:fixed;bottom:2rem;left:50%;transform:translateX(-50%);display:flex;gap:.5rem;z-index:100}.nav-dot{width:12px;height:12px;border-radius:50%;background:#444;cursor:pointer;transition:all .3s}.nav-dot.active{background:#fff;transform:scale(1.2)}.progress-bar{position:fixed;top:0;left:0;height:4px;background:linear-gradient(90deg,#ff6b6b,#feca57);transition:width .3s;z-index:100}.slide-number{position:fixed;bottom:2rem;right:2rem;color:#666;font-size:1rem;z-index:100}.hint{position:fixed;bottom:2rem;left:2rem;color:#444;font-size:.8rem;z-index:100}`,
    ...
    neon: {
        name: '赛博朋克',
        css: `*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;overflow:hidden}body{font-family:'Rajdhani',sans-serif;background:#0a0a0f;color:#00fff2}.slide{height:100vh;display:block;padding:4rem 2rem;overflow-y:auto;align-items:center;padding:4rem;text-align:center;opacity:0;position:absolute;width:100%;transition:opacity .5s}.slide.active{opacity:1}.slide h1{font-family:'Orbitron',sans-serif;font-size:3.5rem;color:#ff00ff;text-shadow:0 0 30px #ff00ff;margin-bottom:2rem}.slide h2{font-size:2rem;color:#00fff2;margin-bottom:1.5rem}.slide p,.slide ul,.slide ol{font-size:1.3rem;color:#e0e0e0}.nav-dots{position:fixed;bottom:2rem;left:50%;transform:translateX(-50%);display:flex;gap:.5rem}.nav-dot{width:10px;height:10px;border-radius:50%;background:#333}.nav-dot.active{background:#ff00ff;box-shadow:0 0 10px #ff00ff}`,
        ...
        render: (slides) => {
            const slidesHtml = slides.map((s, i) => {
                const content = s.content.length ? parseToHtml(s.content.join('\n')) : '';
                return `<div class="slide${i===0?' active':''}">${s.title ? `<h1>${s.title}</h1>` : ''}${content}</div></div>`;
            }).join('');
    ...
    minimal: {
        name: '极简白',
        css: `*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;overflow:hidden}body{font-family:'Inter',sans-serif;background:#fff;color:#000}.slide{height:100vh;display:block;padding:4rem 2rem;overflow-y:auto;align-items:center;padding:4rem;text-align:center;opacity:0;position:absolute;width:100%;transition:opacity .4s}.slide.active{opacity:1}.slide h1{font-size:3rem;font-weight:600;letter-spacing:-.02em;margin-bottom:1.5rem}.slide h2{font-size:1.8rem;font-weight:400;color:#666}.slide p,.slide ul,.slide ol{font-size:1.1rem;color:#888;max-width:600px;line-height:1.8}.nav-dot{width:8px;height:8px;border-radius:50%;background:#ddd}.nav-dot.active{background:#000}`,
    ...
    elegant: {
        name: '暗雅',
        css: `*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;overflow:hidden}body{font-family:'Lato',sans-serif;background:#1a1a1a;color:#f0f0f0}.slide{height:100vh;display:block;padding:4rem 2rem;overflow-y:auto;align-items:center;padding:4rem;text-align:center;opacity:0;position:absolute;width:100%;transition:opacity .6s}.slide.active{opacity:1}.slide h1{font-family:'Playfair Display',serif;font-size:3rem;margin-bottom:1.5rem;background:linear-gradient(135deg,#f0f0f0,#c0c0c0);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.slide h2{font-size:1.6rem;color:#a0a0a0;font-weight:300}.slide p,.slide ul,.slide ol{font-size:1.1rem;color:#808080}`,
